...
for i = 1:sample_size
    global Y_true
    Y_true[i]=0.8*(X1[i,1]+X1[i,2]+X1[i,3])^2+2*sqrt(abs(0.25*X1[i,1]+0.25*X1[i,5]+0.75*X1[i,6]))
end
...
for loop = 1:loop_num
    ...
    outfile = "Projection matrix distance p10 design1 independent normal predictor replication$(datanum).csv"
    f = open(outfile, "a")
    println(f, SDR_Distance)
    close(f)
    outfile2 = "MSE p10 design1 independent normal predictor replication$(datanum).csv"
    f2 = open(outfile2, "a")
    println(f2, mse_train[loop])
    close(f2)
    outfile3 = "HQC p10 design1 independent normal predictor replication$(datanum).csv"
    f3 = open(outfile3, "a")
    println(f3, spline_number)
    println(f3, BIC[loop])
    close(f3)
    outfile4 = "Gelman-Rubin diagnostic p10 design1 independent normal predictor replication$(datanum).csv"
    f4 = open(outfile4, "a")
