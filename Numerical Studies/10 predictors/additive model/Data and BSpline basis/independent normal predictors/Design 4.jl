...
X000=readdlm("sparse10_design4.csv", ',', Float64)
...
for i = 1:sample_size
    global Y_true
    Y_true[i]=2*(0.8*X1[i,1]-0.2*X1[i,3]+0.5*X1[i,6])^2+1.5*exp(0.5*X1[i,1]+0.5*X1[i,4]-0.3*X1[i,5])+8*sin(0.2*X1[i,1]+0.3*X1[i,2]+0.5*X1[i,3]-0.8*X1[i,4])
end

# True indexes and true projection matrix
V1=[8,0,-2,0,0,5,0,0,0,0]
V2=[0.5,0,0,0.5,-0.3,0,0,0,0,0]
V3=[0.2,0.3,0.5,-0.8,0,0,0,0,0,0]
...
for loop = 1:loop_num
    ...
    outfile = "Projection matrix distance p10 design4 independent normal predictor replication$(datanum).csv"
    f = open(outfile, "a")
    println(f, SDR_Distance)
    close(f)
    outfile2 = "MSE p10 design4 independent normal predictor replication$(datanum).csv"
    f2 = open(outfile2, "a")
    println(f2, mse_train[loop])
    close(f2)
    outfile3 = "HQC p10 design4 independent normal predictor replication$(datanum).csv"
    f3 = open(outfile3, "a")
    println(f3, spline_number)
    println(f3, BIC[loop])
    close(f3)
    outfile4 = "Gelman-Rubin diagnostic p10 design4 independent normal predictor replication$(datanum).csv"
    f4 = open(outfile4, "a")
