...
cd("C:/Users/Ksophbrig/My Drive/BMIAM code delivery/10 predictors/additive model/Data and BSpline basis/discrete predictors")
...
X000=readdlm("p10discrete_design5.csv", ',', Float64)

# Number of intervals of B-spline basis function (This should match the 'bsMat')
const spline_number=20

# Load the B-spline basis function (generated from R package:spline)
const bsMat=readdlm("p10discretesplinem$(spline_number).csv", ',', Float64)

# B-spline degree
const B_degree=3

# Equispaced knots points
const knots=collect(-5:(10/spline_number):5)
# Number of Knots
...
for loop = 1:loop_num
    ...
    outfile = "Projection matrix distance p10 design5 discrete uniform predictor replication$(datanum).csv"
    f = open(outfile, "a")
    println(f, SDR_Distance)
    close(f)
    outfile2 = "MSE p10 design5 discrete uniform predictor replication$(datanum).csv"
    f2 = open(outfile2, "a")
    println(f2, mse_train[loop])
    close(f2)
    outfile3 = "HQC p10 design5 discrete uniform predictor replication$(datanum).csv"
    f3 = open(outfile3, "a")
    println(f3, spline_number)
    println(f3, BIC[loop])
    close(f3)
    outfile4 = "Gelman-Rubin diagnostic p10 design5 discrete uniform predictor replication$(datanum).csv"
    f4 = open(outfile4, "a")
