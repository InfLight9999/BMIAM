...
cd("C:/Users/Ksophbrig/My Drive/BMIAM code delivery/10 predictors/non additive model/Data and BSpline basis/discrete predictors")
...
X000=readdlm("nonadditive_p10_design1_discrete.csv", ',', Float64)

# Number of intervals of B-spline basis function (This should match the 'bsMat')
const spline_number=8

# Load the B-spline basis function (generated from R package:spline)
const bsMat=readdlm("discretenonaddsplinem$(spline_number).csv", ',', Float64)

# B-spline degree
const B_degree=3

# Equispaced knots points
const knots=collect(-4:(8/spline_number):4)
# Number of Knots
...
for i = 1:sample_size
    global Y_true
    Y_true[i]=(0.5*X1[i,1]+0.5*X1[i,4]-0.3*X1[i,5])*(1+0.2*X1[i,1]+0.3*X1[i,2]+0.5*X1[i,3]-0.8*X1[i,4])
end
...
for loop = 1:loop_num
    ...
    outfile = "Projection matrix distance p10 nonadditive design1 discrete predictor replication$(datanum).csv"
    f = open(outfile, "a")
    println(f, SDR_Distance)
    close(f)
    outfile2 = "MSE p10 nonadditive design1 discrete predictor replication$(datanum).csv"
    f2 = open(outfile2, "a")
    println(f2, mse_train[loop])
    close(f2)
    outfile3 = "HQC p10 nonadditive design1 discrete predictor replication$(datanum).csv"
    f3 = open(outfile3, "a")
    println(f3, spline_number)
    println(f3, BIC[loop])
    close(f3)
    outfile4 = "Gelman-Rubin diagnostic p10 nonadditive design1 discrete predictor replication$(datanum).csv"
    f4 = open(outfile4, "a")
