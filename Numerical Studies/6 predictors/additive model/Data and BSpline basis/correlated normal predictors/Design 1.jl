...
X000=readdlm("correlated_design1.csv", ',', Float64)
...
for loop = 1:loop_num
    ...
    outfile = "Projection matrix distance p6 design1 correlated normal predictor replication$(datanum).csv"
    f = open(outfile, "a")
    println(f, SDR_Distance)
    close(f)
    outfile2 = "MSE p6 design1 correlated normal predictor replication$(datanum).csv"
    f2 = open(outfile2, "a")
    println(f2, mse_train[loop])
    close(f2)
    outfile3 = "HQC p6 design1 correlated normal predictor replication$(datanum).csv"
    f3 = open(outfile3, "a")
    println(f3, spline_number)
    println(f3, BIC[loop])
    close(f3)
    outfile4 = "Gelman-Rubin diagnostic p6 design1 correlated normal predictor replication$(datanum).csv"
    f4 = open(outfile4, "a")
