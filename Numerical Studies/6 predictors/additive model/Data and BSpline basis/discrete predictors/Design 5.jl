...
X000=readdlm("simu_discrete5.csv", ',', Float64)

# Number of intervals of B-spline basis function (This should match the 'bsMat')
const spline_number=30

# Load the B-spline basis function (generated from R package:spline)
const bsMat=readdlm("discreteknotssplinem$(spline_number).csv", ',', Float64)

# B-spline degree
const B_degree=3

# Equispaced knots points
const knots=collect(-4.2:(8.4/spline_number):4.2)
# Number of Knots
...
const knot_farleft_position=-4.2
...
for loop = 1:loop_num
    ...
    outfile = "Projection matrix distance p6 design5 discrete predictor replication$(datanum).csv"
    f = open(outfile, "a")
    println(f, SDR_Distance)
    close(f)
    outfile2 = "MSE p6 design5 discrete predictor replication$(datanum).csv"
    f2 = open(outfile2, "a")
    println(f2, mse_train[loop])
    close(f2)
    outfile3 = "HQC p6 design5 discrete predictor replication$(datanum).csv"
    f3 = open(outfile3, "a")
    println(f3, spline_number)
    println(f3, BIC[loop])
    close(f3)
    outfile4 = "Gelman-Rubin diagnostic p6 design5 discrete predictor replication$(datanum).csv"
    f4 = open(outfile4, "a")
