...
cd("C:/Users/Ksophbrig/My Drive/BMIAM code delivery/6 predictors/additive model/Data and BSpline basis/independent normal predictors")
...
X000=readdlm("p6_indepNormal_design5.csv", ',', Float64)

# Number of intervals of B-spline basis function (This should match the 'bsMat')
const spline_number=16

# Load the B-spline basis function (generated from R package:spline)
const bsMat=readdlm("knotssplinem$(spline_number).csv", ',', Float64)

# B-spline degree
const B_degree=3

# Equispaced knots points
const knots=collect(-10:(20/spline_number):10)
# Number of Knots
...
V1=[0.5,0,0,0.5,-0.3,0]
V2=[0.2,0.3,0.5,-0.8,0,0]
V3=[0.8,0,-0.2,0,0,0.5]
...
const knot_farleft_position=-10
...
for loop = 1:loop_num
    ...
    outfile = "Projection matrix distance p6 design5 independent normal predictor replication$(datanum).csv"
    f = open(outfile, "a")
    println(f, SDR_Distance)
    close(f)
    outfile2 = "MSE p6 design5 independent normal predictor replication$(datanum).csv"
    f2 = open(outfile2, "a")
    println(f2, mse_train[loop])
    close(f2)
    outfile3 = "HQC p6 design5 independent normal predictor replication$(datanum).csv"
    f3 = open(outfile3, "a")
    println(f3, spline_number)
    println(f3, BIC[loop])
    close(f3)
    outfile4 = "Gelman-Rubin diagnostic p6 design5 independent normal predictor replication$(datanum).csv"
    f4 = open(outfile4, "a")
