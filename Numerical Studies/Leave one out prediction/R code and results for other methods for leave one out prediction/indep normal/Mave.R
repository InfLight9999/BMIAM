...
{
  dim.mave=2
  dr.mave <- mave(c[1:(sample.size-1),7,turn]~c[1:(sample.size-1),1:6,turn], method = 'MEANMAVE', max.dim = dim.mave)
  x.train.mave <- mave.data(dr.mave, x = c[1:(sample.size-1),1:6,turn] , dim = dim.mave)
  x.test.mave <- mave.data(dr.mave,x = c[sample.size,1:6,turn], dim = dim.mave)
  model.mars <- mars(x.train.mave, c[1:(sample.size-1),7,turn], degree=dim.mave)
  y.pred.mars <- predict(model.mars, x.test.mave)
  mse_mave[turn]<-mean((y.pred.mars - c[sample.size,7,turn])^2)
}
...
write.csv(as.data.frame(result.output), "mave performance.csv", row.names=FALSE)