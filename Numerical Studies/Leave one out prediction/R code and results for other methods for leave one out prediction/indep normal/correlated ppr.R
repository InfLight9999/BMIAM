...
for (j in 1:100)
{
  ppr.fit<-ppr(x=c[(1+sample.size*(j-1)):(sample.size*j-1),1:6], y=c[(1+sample.size*(j-1)):(sample.size*j-1),7],nterms=2)
  ppr.gcvsplinefit<-ppr(x=c[(1+sample.size*(j-1)):(sample.size*j-1),1:6], y=c[(1+sample.size*(j-1)):(sample.size*j-1),7],nterms=2, sm.method = "gcvspline")
  mse[j]<-mean((predict(ppr.fit,newdata = t(c[(sample.size*j),1:6]))-c[(sample.size*j),7])^2)
  mse.gcvspline[j]<-mean((predict(ppr.gcvsplinefit,newdata = t(c[(sample.size*j),1:6]))-c[(sample.size*j),7])^2)
}
...
write.csv(as.data.frame(result.output), "ppr performance.csv", row.names=FALSE)