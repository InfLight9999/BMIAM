...
mse_train=sum((Y_true-eta).^2)/sample_size
...
outfile = "../result/Projection_matrix_distance_setting_$(SETTING)_p$(DIM)_design$(DESIGN)_rep$(datanum)_within.csv"
f = open(outfile, "a")
println(f, [datanum,SDR_Distance,datanum])
close(f)

outfile2 = "../result/MSE_$(SETTING)_p$(DIM)_design$(DESIGN)_rep$(datanum)_within.csv"
f2 = open(outfile2, "a")
println(f2, [datanum,mse_train,datanum])
close(f2)

outfile3 = "../result/HQC_setting$(SETTING)_p$(DIM)_design$(DESIGN)_rep$(datanum)_within.csv"
f3 = open(outfile3, "a")
println(f3, [datanum,spline_number,mBIC,datanum])
close(f3)

outfile4 = "../result/Gelman_Rubin_setting$(SETTING)_p$(DIM)_design$(DESIGN)_rep$(datanum)_within.csv"
f4 = open(outfile4, "a")
println(f4, [datanum,maximum(theta_R_hat),maximum(beta_R_hat),datanum])
close(f4)
